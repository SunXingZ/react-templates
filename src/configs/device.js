...
const statusBarHeight =
	platform == 'ios'
		? iphoneStatusBar + iphoneXStatusBar + iphoneXsMaxStatusBar
		: isAndroidFullScreen
		? baseAndroidStatusBarHeight
		: baseStatusBarHeight;
...
const navBarHeight =
	platform == 'ios'
		? iphoneNavBar + iphoneXNavBar + iphoneXsMaxNavBar
		: isAndroidFullScreen
		? baseAndroidStatusBarHeight + baseNavBarHeight
		: baseStatusBarHeight + baseNavBarHeight;
...
const requestAndroidPermission = async (permission) => {
	...
	if (!hasPermission) {
		hasPermission = await PermissionsAndroid.requestPermission(permission)
			.then((response) => response)
			.catch((error) => false);
	}
...
const requestMultipleAndroidPermission = async (permissionArray) => {
	return await PermissionsAndroid.requestMultiple(permissionArray)
		.then((response) => response)
		.catch((error) => false);
};
